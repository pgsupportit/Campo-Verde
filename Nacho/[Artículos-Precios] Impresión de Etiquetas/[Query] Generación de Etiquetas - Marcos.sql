...
-- * Si esta "Impreso" = S o N
-- ******Filtros:
-- ** Se filtra por los que son "Tipo no controla" y los "Unitarios"
...
WHERE STA11.CAMPOS_ADICIONALES.value('(/CAMPOS_ADICIONALES/CA_INFORMACION_PARA_BALANZA)[1]', 'NVARCHAR(MAX)') in ('TIPO NO CONTROLA', 'TIPO PLU N si es UNITARIO') -- Filtro para solo traer los "Tipo No Controla" y "Si es Unitario"
...
AND (
  ...
  OR GVA17.FECHA_MODI = '' -- Filtro por rango de fecha de modificación del precio (como ejemplo)
);
...
-- * Si esta impreso = S o N
-- ******Filtros:
-- ** Se filtra por los que son "Tipo no controla" y "Unitarios"
-- ** Se filtra por Lista de Precios = '10'.
-- ** Se trae los Precios que aún no fueron impresos.
SELECT
  STA11.COD_ARTICU AS [CÓDIGO DE ARTÍCULO],
  STA11.DESCRIPCIO AS [DESCRIPCIÓN ARTÍCULO], -- Descripción del artículo
  STA11.COD_BARRA AS [CÓDIGO DE BARRAS], -- Código de Barras
...
WHERE STA11.CAMPOS_ADICIONALES.value('(/CAMPOS_ADICIONALES/CA_INFORMACION_PARA_BALANZA)[1]', 'NVARCHAR(MAX)') in ('TIPO NO CONTROLA', 'TIPO PLU N si es UNITARIO') AND COD_ARTICU = ''
